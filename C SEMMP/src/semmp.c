...
/* ------------------------------------------------------------------------- */
#include <stdio.h>			// IOSEMMP.H
#include <stdlib.h>
#include <math.h>			// GRID.H 			// WELL.H
#include <string.h>			// IOSEMMP.H 		// WELL.H
#include <malloc.h> 		// MEMORY.H
#include <limits.h>
#include <float.h>
#include "umfpack.h"		// GRID.H 			// SEMMP.H
#include "iniparser.h" 		// IOSEMMP.H 		// WELL.H
#include "eprintf.h" 		// BOUNDARY.H 		// MEMORY.H 	// GRID.H 		// IOSEMMP.H 		// WELL.H 		// SEMMP.H
#include "semmp.h" 			
...
double *rVector(int size) 																	// MEMORY.H
{
	double *v;
...
double **rMatrix(int rows, int cols) 														// MEMORY.H
{
	int i;
	double **m;
...
int **iMatrix(int rows, int cols) 															// MEMORY.H
{
	int i;
	int **m;
...
void freeiVector(int *v) 																	// MEMORY.H
{
	free(v);
...
void freerVector(double *v) 																// MEMORY.H
{
	free(v);
...
void freeiMatrix(int **m, int rows, int cols) 												// MEMORY.H
{
	int i;
...
void freerMatrix(double **m, int rows, int cols) 											// MEMORY.H
{
	int i;
...
void rTableFile(char filein[], double **table, int nrow, int ncol) 							// NEW FUNCTION IN IOSEMMP.H ????
{
	float tmp;
	int col, row;
	FILE *mf;
...
void iTableFile(char filein[], int **table, int nrow, int ncol) 							// NEW FUNCTION IN IOSEMMP.H ????
{
	float tmp;
	int col, row;
	FILE *mf;
...
Block *readAndSetGeometry(Parameters *par, int **geom)										// IOSEMMP.H
{
...
void setCartesianTransmissibilities(Parameters *par, Block *grid, Boundary *boundary)		// GRID.H
{
	int i, j, bcIndex;
...
void setCylindricalTransmissibilities(Parameters *par, Block *grid, Boundary 				// GRID.H
								 *boundary, Well *wells)
{
	int i, j, n, bcIndex;
	int bool_E, bool_W, bool_N, bool_S;
...
double **readFluidProperties(Parameters *par)												// IOSEMMP.H
...
void setInitialPressure(Parameters *par, Block *grid, Well *wells, 							// GRID.H
...
double normDelta(double *a, double *b, int n)												// GRID.H
{
	int i;
...
double getTableVal(double **table, int colx, double x, int colfx, double delta, 			// IOSEMMP.H
				   int rowmax)
...
void setMatrixStructure(Parameters *par, Block *grid, int *Ti, int *Tj, 					// GRID.H
...
void setTransmissibilityMatrix(Parameters *par, Block *grid, Well *wells,					// GRID.H
...
void readIniFile(int argc, char *argv[], Parameters *par)									// IOSEMMP.H
...
Well *readAndSetWellParameters(Parameters *par)												// IOSEMMP.H
{
	Well *wells;
	dictionary *ini;
	int i, nsec;
...
void writePressureFile(Parameters *par, int step, Block *grid, double *pressure)			// IOSEMMP.H
{
	FILE *fp;
...
void setWells(Parameters *par, Block *grid, int **geom, Well *wells)						// GRID.H
{
	int i, j, localIndex;
...
double reqPeaceman(Block *grid, int localIndex) 											// WELL.H
...
double reqDing(Block *grid, int localIndex, double req) 									// WELL.H
...
double reqAbouKassemAziz(Block *grid, int localIndex) 										// WELL.H
...
void writeOuts(FILE* local_write, int nBlocksOut, Block* grid, Well *wells, 				// IOSEMMP.H
...
void header(FILE *file, Parameters *par)													// IOSEMMP.H
{
	fprintf(file,"\
   =========================================================================  \n\n\
...
void writeTableHeader(FILE *local_out, Parameters *par, Out *outs, int n_outs,				// IOSEMMP.H
...
Out *readAndSetOuts(Parameters *par, Block *grid, int **geom, char *type, 					// IOSEMMP.H
					double *cumulativeProd)
{
	dictionary *ini;
	Out *outs;
...
void *readAndSetMainOut(Parameters *par, double *cumulativeProd)							// IOSEMMP.H
{
	dictionary *ini;
	char dirFileName[LENGTHFN];
...
void modifyGeometry(int **geom, Parameters *par){											// IOSEMMP.H
	FILE *mf;
...
void modifyTransmissibilities(int **geom, Block *grid, Parameters *par){					// IOSEMMP.H
	FILE *mf;
...
Boundary *readAndSetBoundaryConditions(Parameters *par, int **geom, Block *grid)			// IOSEMMP.H
{
	Boundary *boundary;
	dictionary *ini;
...
void setBoundaryConditions(int *j, int *Map, double *Ax, double *b, 						// BOUNDARY.H
